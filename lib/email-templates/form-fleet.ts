/**
 * Email Templates — form-fleet
 * ==============================
 * Fleet Managers landing page enquiries.
 */
...
export const adminTemplate: EmailTemplate = {
  subject: (data) =>
    `[form-fleet] New Fleet Managers enquiry${data.company ? ` — ${data.company}` : ""}`,

  html: (data) =>
    emailLayout({
      preheader: `New fleet enquiry from ${data.company || data.name || "a visitor"}`,
      body: `
        <div style="background: ${BRAND.primary}; color: white; padding: 24px 32px;">
          <h2 style="margin: 0; font-weight: 500; font-size: 22px;">New Fleet Enquiry</h2>
          <p style="margin: 4px 0 0; font-family: monospace; font-size: 12px; opacity: 0.85;">form-fleet</p>
        </div>
        <div style="padding: 32px;">
          <p style="margin: 0 0 16px; color: ${BRAND.text}; font-size: 16px;">
            A fleet manager has submitted an enquiry on the website.
          </p>
          ${detailsTable([
            ["Name", data.name],
            ["Email", data.email],
...
export const customerTemplate: EmailTemplate = {
  subject: () => "Thank you for your fleet enquiry — EV360",
    ...
    emailLayout({
      ...
      body: `
        <div style="padding: 40px 32px;">
          ...
          <p style="margin: 0 0 16px; color: ${BRAND.textMuted}; font-size: 16px; line-height: 1.6;">
            Thank you for your interest in our fleet management solutions.
            Our partnerships team will review your enquiry and be in touch within
            24 hours to discuss how we can help optimise your electric fleet lifecycle.
          </p>
